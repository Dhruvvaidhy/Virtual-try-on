...
export default function VirtualFitTool() {
  ...
  const handleDownload = () => {
    if (!user) {
      alert("You need to log in or sign up to download the image.");
      return;
    }  
        ...
        <div className="w-full md:w-1/3 border-b md:border-b-0 md:border-r border-white/20 pb-4 md:pb-0">
          ...
          <div className="border border-white/20 bg-white/30 backdrop-blur-lg p-4 rounded-lg text-center mb-4">
            ...
            <div className="border-2 border-white/20 p-4 rounded-lg relative h-40 flex items-center justify-center">
              {modelImage ? <img src={modelImage} alt="Model" className="max-w-full max-h-full object-contain rounded-lg" /> : <p className="text-gray-200">No Image Uploaded</p>}
              <div className="absolute bottom-2 flex gap-3">
                <label className="p-2 bg-gray-200 rounded-full cursor-pointer">
                  <FaUpload size={20} />
                  <input type="file" accept="image/*" className="hidden" id="modelImageInput" onChange={(e) => handleImageUpload(e, setModelImage)} />
                </label>
                {modelImage && <button className="p-2 bg-red-500 text-white rounded-full" onClick={() => handleDeleteImage(setModelImage)}><FaTrash size={20} /></button>}
              </div>
            </div>
          </div>
          ...
          <div className="border border-white/20 bg-white/30 backdrop-blur-lg p-4 rounded-lg text-center mb-4">
            ...
            <div className="border-2 border-white/20 p-4 rounded-lg relative h-40 flex items-center justify-center">
              {clothesImage ? <img src={clothesImage} alt="Clothes" className="max-w-full max-h-full object-contain rounded-lg" /> : <p className="text-gray-200">No Image Uploaded</p>}
              <div className="absolute bottom-2 flex gap-3">
                <label className="p-2 bg-gray-200 rounded-full cursor-pointer">
                  <FaUpload size={20} />
                  <input type="file" accept="image/*" className="hidden" id="clothesImageInput" onChange={(e) => handleImageUpload(e, setClothesImage)} />
                </label>
                {clothesImage && <button className="p-2 bg-red-500 text-white rounded-full" onClick={() => handleDeleteImage(setClothesImage)}><FaTrash size={20} /></button>}
              </div>
            </div>
          </div>
          ...
          {/* Generate Button */}
          <button className="w-full bg-purple-600 text-white p-3 rounded-lg mt-4" onClick={handleGenerate} disabled={loading}>
...
<div className="w-full md:w-2/3 text-center">
  <p className="text-lg font-semibold text-white">Generated Image</p>
    ...
    {loading ? (
      ...
      <>
        ...
        <div className="absolute top-2 right-2 flex gap-2">
          {/* Download Button */}
          <button className="bg-green-600 text-white p-3 rounded-full flex items-center cursor-pointer" onClick={handleDownload}>
            <FaDownload size={20} />
          </button>
          {/* Delete Button */}
          <button className="bg-red-600 text-white p-3 rounded-full flex items-center cursor-pointer" onClick={handleDeleteOutput}>
            <FaTrash size={20} />
          </button>
        </div>
      </>
    ) : (
      <p className="text-gray-200">No Image Generated</p>
    )}
  </div>
</div>
...
<div className="w-full md:w-2/3 text-center">
  ...
  <div className="border border-white/20 bg-white/20 backdrop-blur-lg p-4 rounded-lg mt-2 h-104 flex items-center justify-center relative">
    {loading ? (
      ...
      <>
        <img src={outputImage} alt="Output" className="max-w-full max-h-full object-contain rounded-lg" />
        <button className="absolute top-2 right-2 bg-green-600 text-white p-3 rounded-full flex items-center cursor-pointer" onClick={handleDownload}>
          <FaDownload size={20} />
        </button>
      </>
    ) : (
      <p className="text-gray-200">No Image Generated</p>
        ...
        <div className="w-full md:w-2/3 text-center">
          <p className="text-lg font-semibold text-white">Generated Output</p>
          <div className="border border-white/20 bg-white/20 backdrop-blur-lg p-4 rounded-lg mt-2 h-104 flex items-center justify-center relative">
            {outputImage && <img src={outputImage} alt="Output" className="max-w-full max-h-full object-contain rounded-lg" />}
            <button className="absolute top-2 right-2 bg-green-600 text-white p-3 rounded-full flex items-center cursor-pointer" onClick={handleDownload}>
              <FaDownload size={20} />
            </button>
          </div>
...
//   const handleDownload = () => {
//     if (!user) {
//       alert("Please login to download the generated image.");
//       return;
//     }
